...
void profiler_finalise()
{
  ...
  *profiler_count = words_written;
  io_printf(IO_BUF, "Profiler wrote %u bytes to %08x.", (words_written * 4) + 4, profiler_count);
}
//---------------------------------------
void profiler_init(uint32_t num_samples)
{
  io_printf(IO_BUF, "Initialising profiler with storage for %u samples", num_samples);
  
  // Initialize number of samples remaining
  profiler_samples_remaining = num_samples;
  
  // Start timer 2 with no clock divider
  tc[T2_CONTROL] = 0x82;
  tc[T2_LOAD] = 0;
}
