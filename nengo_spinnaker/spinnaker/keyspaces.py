...
class Keyspace(object):
    ...
    integral) fields. For example, there might be a field which specifies
    which chip a message is destined for, another which specifies the core and
    ...
    We can also specify just some keys at a time. This means that the fields can
    be specified at different points in execution where it makes most sense. As
    an example::
    ...
    `external`, `device_id` and `command`. In this example, the `device_id` and
    `command` fields are free to overlap with the `chip`, `core` and `type`
    fields since they are never present in the same key.
    ...
    value ever assigned to that field up until that point in the program. In the
    above example, that means the fields `chip`, `core` and `type` will have
    been assigned a length of 1. Attempting to define a key with these fields
    larger than 1 will now fail. As a result, users are advised to define all
    field values in one phase of execution and then generate keys in a second
    distinct phase.
    ...
    easily generate keys/masks based only on these fields::
        ...
        # Keys/masks for a device (note that we don't need to define the command
        # field since it does not have the routing tag.
    ...
    def add_field(self, identifier, length = None, start_at = None, tags = None):
        ...
        :py:class:`ValueError`
            ...
            positions do not undergo such checks until their length and position
            become known.
        """
    ...
    def get_mask(self, tag = None, field = None):
        ...
        tag : str
            ...
            the specified tag.
        field : str
            Optionally specifies that the mask should only include the specified
            field.
    ...
    def _enabled_fields(self, field_values = None):
        ...
        field_values : dict or None
            ...
            If *None*, uses `self.field_values`.
        """
    ...
    def _potential_fields(self, field_values = None):
        ...
        # dependency.  Simply accumulate blocked fields which either depend on a
        # field value which has been defined as a non-matching value or depend
        # on fields which have already been blocked.
        blocked = set()
        for identifier, field in self.fields.items():
            if field.conditions:
                met = True
